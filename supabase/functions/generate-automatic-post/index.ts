...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (schedule.category_ids && schedule.category_ids.length > 0) {
        ...
        if (catError) console.error(`Failed to associate categories for post ${newPost.id}:`, catError.message);
        else console.log(`Associated ${postCategories.length} categories to post ${newPost.id}.`);
    }
    ...
    if (internalSecret) {
      fetch(translateUrl, {
        ...
        if (!res.ok) console.error(`Failed to trigger translation for post ${newPost.id}. Status: ${res.status}`);
        else console.log(`Translation successfully triggered for post ${newPost.id}.`);
      }).catch(err => console.error(`Error triggering translation for post ${newPost.id}:`, err));
    } else {

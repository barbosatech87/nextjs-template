...
async function refineContentWithClaude(content: string): Promise<string> {
  console.log("[CLAUDE_REFINE] Starting content refinement.");
  if (!process.env.CLAUDE_API_KEY) {
    console.warn("[CLAUDE_REFINE] CLAUDE_API_KEY not set. Skipping refinement step.");
    return content;
  }
  ...
  try {
    ...
    console.log("[CLAUDE_REFINE] Content successfully refined by Claude.");
    return refinedContent;

  } catch (error) {
    console.error("[CLAUDE_REFINE] Error during refinement:", error);
    return content;
  }
}
...
export async function generatePostWithAI(
  ...
  console.log("[AI_POST_FLOW] Starting AI post generation with request:", request);
  ...
  try {
    ...
    console.log(`[AI_POST_FLOW] Draft generated by OpenAI: "${parsedContent.title}"`);
    console.log(`[AI_POST_FLOW] OpenAI Raw Content (before Claude):`, parsedContent.content.substring(0, 150) + "...");

    if (parsedContent.content) {
        console.log("[AI_POST_FLOW] Calling Claude for content refinement...");
        parsedContent.content = await refineContentWithClaude(parsedContent.content);
        console.log(`[AI_POST_FLOW] Claude Refined Content:`, parsedContent.content.substring(0, 150) + "...");
    }

    console.log("[AI_POST_FLOW] Successfully generated and refined post. Returning data.");
    return { success: true, data: parsedContent };
  } catch (error) {
    console.error("[AI_POST_FLOW] Error during post generation:", error);

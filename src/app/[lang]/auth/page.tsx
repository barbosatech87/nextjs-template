...
export default function AuthPage({ params }: AuthPageProps) {
  ...
  const currentTitle =
    TITLES[lang]?.[view] ?? TITLES["pt"][view];

  const handleSelectView = (nextView: AuthView) => {
    setView(nextView);
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-muted p-4">
      <div className="w-full max-w-md p-8 bg-background shadow-lg rounded-lg border">
        <h1 className="text-2xl font-bold mb-6 text-center">{currentTitle}</h1>
        ...
        {view === "sign_up" ? (
          <SignUpForm
            lang={lang}
            onSwitchToSignIn={() => handleSelectView("sign_in")}
          />
        ) : (
          <Auth
